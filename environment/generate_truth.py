...
__all__ = [
    ...
    "sample_sources", "gaussian_field", "inverse_square_field", "get_param"
]
...
def inverse_square_field(
    ...
    """Return (H, W) radiation field using 3D inverse square law.

    This matches the physics model used in ADE-PSPF prediction (Equation 1).

    I(x, y) = Σ_k [A_k / (h² + d_k²)] * exp(-μ * r_k)

    where:
        A_k = source strength
        d_k = 2D distance from source k (meters)
        h = height of source above observation plane (meters)
        r_k = 3D distance = sqrt(d_k² + h²)
        μ = air absorption coefficient (≈0 for short distances)

    Args:
        grid: Grid size (pixels)
        coords: Source positions [(y, x), ...] in pixels
        amps: Source amplitudes [A, ...] (intensity units)
        h: Height of sources (meters, default 0.5)
        mu_air: Air absorption coefficient (default 0.0, approximated as negligible)

    Returns:
        field: (grid, grid) array of radiation intensities
    """
    # Conversion factor: 256 pixels = 10 meters
    pixel_to_meter = 10.0 / grid
    ...
    for (y0, x0), A in zip(coords, amps, strict=True):
        ...
        dy_meters = (yy - y0) * pixel_to_meter
        dx_meters = (xx - x0) * pixel_to_meter
        d2_meters = dy_meters**2 + dx_meters**2

        # 3D distance with height
        r2 = d2_meters + h**2

        # Inverse square law (exp term ≈ 1 for μ≈0)
        # Avoid division by zero at source center
        r2 = np.maximum(r2, h**2)  # Minimum distance is h
        field += A / r2
